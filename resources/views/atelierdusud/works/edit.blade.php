<x-app-layout>
    ...
    <div class="bg-white max-w-5xl mx-auto my-12 p-8 shadow-xl rounded-sm">
        <form action="{{ route('works.update', $work) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="">
                ...
                <div class="mt-2">
                    <input type="text" name="title" id="title" value="{{ $errors->has('title') ? old('title') : $work->title }}" placeholder="Entrez le nom de l'oeuvre" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
                    @error('title')
            ...
            <div class="">
                ...
                <select class="mt-2 block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6" name="category_id" id="category">
                    ...
                    <option value="{{ $category->id }}" @if ($category->id == $work->category_id) selected @endif>{{ $category->name }}{{ $category->name }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <div class="mt-2 flex items-center gap-x-1 cursor-pointer rounded-full px-2 border {{ in_array($artist->id, $work->artists->pluck('id')->toArray()) ? 'bg-blue-600 text-white ' : 'bg-gray-50 text-gray-900' }}">
                        <input type="checkbox" name="artists[]" id="artist_{{ $artist->id }}" value="{{ $artist->id }}"
                        @checked(in_array($artist->id, $work->artists->pluck('id')->toArray()))
                        class="rounded-md border-gray-300 shadow-sm focus:border-blue-300 cursor-pointer focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            ...
            <div class="">
                ...
                <img src="{{ url($work->url) }}" alt="{{ $work->url }}" class="h-44 w-44 rounded-full object-cover">
            ...
            <div class="flex items-center" x-data="{ on: {{ $work->status }} }">
